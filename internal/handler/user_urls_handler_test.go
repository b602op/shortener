...
func newTestServer(t *testing.T) *httptest.Server {
	...
	deleteService := worker.NewDeleteService(store, worker.Config{
		WorkerCount:    1,
		QueueSize:      16,
		BufferSize:     4,
...
func TestDeleteUserURLs_ForeignUser(t *testing.T) {
	...
	// 2. Второй пользователь — получаем его куку через отдельный POST
	...
	require.NotEqual(t, ownerCookies[0].Value, otherUserCookies[0].Value,
		"второй пользователь должен получить другую куку")

	// 3. Второй пользователь пытается удалить чужой URL
	body := fmt.Sprintf(`["%s"]`, shortID)
	req, err := http.NewRequest(http.MethodDelete, ts.URL+"/api/user/urls", strings.NewReader(body))
	require.NoError(t, err)
	req.AddCookie(otherUserCookies[0])
